 ...
 */
class OEGravityFormsStyler extends \OxyExtendedEl {

	public $css_added = false;

	/**
	 * Retrieve Gravity Forms element name.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function name() {
		return __( 'Gravity Forms Styler', 'oxy-extended' );
	}

	/**
	 * Retrieve Gravity Forms element slug.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function slug() {
		return 'oe_gravity_forms_styler';
	}
	...
	/**
	 * Retrieve Gravity Forms element icon.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function controls() {
		$select_form = $this->addOptionControl(
			array(
				...
				'slug'    => 'oe_select_form',
				'value'   => OE_Helper::get_contact_forms( 'Gravity_Forms' ),
				'default' => '-1',
				'css'     => false,
			)
		);
		...
		$form_title = $this->typographySection( __( 'Form Title', 'oxy-extended' ), '.gform_title', $this );
		...
		$form_desc = $this->typographySection( __( 'Form Description', 'oxy-extended' ), '.gform_description', $this );
	...
	/**
	 * Render Gravity Forms element output on the frontend.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function render( $options, $defaults, $content ) {
		...
		?>
		<div class = "oe-gravity-forms-container">
			<div class = "oe-contact-form oe-gravity-forms">
				<?php
				if ( $options['oe_select_form'] ) {
					$title = ( ! empty( $options['gform_title'] ) && 'no' === $options['gform_title'] ) ? 'false' : 'true';
					$desc = ( ! empty( $options['gform_desc'] ) && 'no' === $options['gform_desc'] ) ? 'false' : 'true';
					$ajax = ( ! empty( $options['gform_ajax'] ) && 'no' === $options['gform_ajax'] ) ? 'false' : 'true';
					$tab_index = ! empty( 'yes' === $options['gform_tabi'] ) ? $options['gform_tabi'] : 10;

					if ( defined( 'OXY_ELEMENTS_API_AJAX' ) && isset( $options['oe_select_form'] ) ) {
						$form = \GFAPI::get_form( $options['oe_select_form'] );
						\GFFormDisplay::print_form_scripts( $form, $ajax );
					}

					echo do_shortcode( '[gravityform id=' . $options['oe_select_form'] . ' title="' . $title . '" description="' . $desc . '" ajax="' . $ajax . '" tabindex="' . $tab_index . '"]' );
				}
				?>
			</div>
		</div>
	...
	public function customCSS( $original, $selector ) {
		...
		if ( isset( $original[ $prefix . '_oe_radio_check_smart_ui' ] ) && $original[ $prefix . '_oe_radio_check_smart_ui' ] == 'yes' ) {
...
( new OEGravityFormsStyler() )->removeApplyParamsButton();
