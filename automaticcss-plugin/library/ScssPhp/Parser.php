...
class Parser {
	...
	public function parseError( $msg = 'parse error' ) {
		...
		if ( $this->peek( '(.*?)(\n|$)', $m, $this->count ) ) {
			...
			$e->setSourcePosition( array( $this->sourceName, $line, $column ) );
	...
	protected function parseChunk() {
		...
		if ( isset( $this->buffer[ $this->count ] ) && $this->buffer[ $this->count ] === '@' ) {
			...
			if (
				...
				if ( $this->cssOnly ) {
					$this->assertPlainCssValid( array( Type::T_IMPORT, $importPath ), $s );
					$this->append( array( Type::T_COMMENT, rtrim( substr( $this->buffer, $s, $this->count - $s ) ) ) );
					return true;
				}
			...
			if (
				...
				if ( count( $dirName ) === 1 && is_string( reset( $dirName ) ) ) {
					...
					$dirName = array( Type::T_STRING, '', $dirName );
				}
				if ( $dirName === 'media' ) {
			...
			if (
				...
				if (
					...
					$this->env->type &&
					! \in_array( $this->env->type, array( Type::T_DIRECTIVE, Type::T_MEDIA ) )
				) {
				...
				$this->append( array( Type::T_DIRECTIVE, array( $dirName, $dirValue, $hasBlankLine, $isNotRoot ) ), $s );
				$this->whitespace();
		...
		if ( ( $this->customProperty( $name ) ) && $this->matchChar( ':', false ) ) {
			...
			foreach ( array( ';', '}' ) as $ending ) {
				if (
					...
					$nestingPairs = array( array( '[', ']' ), array( '{', '}' ) );
					...
					$this->seek( $end );
					$this->append( array( Type::T_CUSTOM_PROPERTY, $name, $value ), $s );
	...
	protected function isPlainCssValidElement( $parsed, $allowExpression = false ) {
		...
		if (
			\in_array( $parsed[0], array( Type::T_FUNCTION, Type::T_FUNCTION_CALL ) ) &&
			! \in_array(
				$parsed[1],
				array(
					'alpha',
		...
		switch ( $parsed[0] ) {
			...
			case Type::T_EXPRESSION:
				...
				if ( ! $allowExpression && ! \in_array( $op, array( 'and', 'or', '/' ) ) ) {
					return false;
				}
			...
			case Type::T_FUNCTION_CALL:
				...
				$argsList = array( Type::T_LIST, ',', array() );
	...
	protected function matchString( &$m, $delim ) {
		...
		foreach ( array( '#{', '\\', "\r", $delim ) as $lookahead ) {
			...
			if ( isset( $m[1] ) && empty( $this->commentsSeen[ $this->count ] ) ) {
				...
				while ( $p !== false && $p < $endCommentCount ) {
					...
					if ( $this->interpolation( $out ) ) {
						...
						$comment[] = array( Type::T_COMMENT, substr( $this->buffer, $p, $this->count - $p ), $out );
					} else {
						list($line, $column) = $this->getSourcePosition( $this->count );
						$file = $this->sourceName;
				...
				if ( ! $comment ) {
					...
					$commentStatement = array( Type::T_COMMENT, $staticComment, array( Type::T_STRING, '', $comment ) );
	...
	protected function expression( &$out, $listOnly = false, $lookForExp = true ) {
		...
		$this->discardComments = true;
		$allowedTypes = ( $listOnly ? array( Type::T_LIST ) : array( Type::T_LIST, Type::T_MAP ) );
		...
		if ( \in_array( Type::T_LIST, $allowedTypes ) && $this->matchChar( '[' ) ) {
			if ( $this->enclosedExpression( $lhs, $s, ']', array( Type::T_LIST ) ) ) {
				if ( $lookForExp ) {
					$out = $this->expHelper( $lhs, 0 );
				} else {
					$out = $lhs;
	 ...
	 * @return boolean
	 */
	protected function enclosedExpression( &$out, $s, $closingParen = ')', $allowedTypes = array( Type::T_LIST, Type::T_MAP ) ) {
		...
		if (
			...
			\in_array( $out[0], array( Type::T_EXPRESSION, Type::T_UNARY ) ) ) &&
			\in_array( Type::T_LIST, $allowedTypes )
		) {
			if ( $out[0] !== Type::T_LIST || ! empty( $out['enclosing'] ) ) {
				$out = array( Type::T_LIST, '', array( $out ) );
			}
	...
	protected function color( &$out ) {
		...
		if ( $this->match( '(#([0-9a-f]+)\b)', $m ) ) {
			if ( \in_array( \strlen( $m[2] ), array( 3, 4, 6, 8 ) ) ) {
				$out = array( Type::T_KEYWORD, $m[0] );
	...
	protected function matchEscapeCharacter( &$out, $inKeywords = false ) {
		...
		if ( $this->match( '.', $m, false ) ) {
			if ( $inKeywords && in_array( $m[0], array( "'", '"', '@', '&', ' ', '\\', ':', '/', '%' ) ) ) {
				$this->seek( $s );
				return false;
			}
			$out = $m[0];
			...
			if ( $char === ':' ) {
				...
				if ( $this->mixedKeyword( $nameParts, true ) ) {
					...
					if (
						...
						$nameParts === array( 'is' ) ||
						$nameParts === array( 'has' ) ||
						$nameParts === array( 'where' ) ||
						$nameParts === array( 'slotted' ) ||
						$nameParts === array( 'nth-child' ) ||
						$nameParts === array( 'nth-last-child' ) ||
						$nameParts === array( 'nth-of-type' ) ||
						$nameParts === array( 'nth-last-of-type' )
					) {
						if (
							$this->matchChar( '(', true ) &&
	...
	protected function url( &$out ) {
		if ( $this->literal( 'url(', 4 ) ) {
			...
			if (
				...
				$out = array( Type::T_STRING, '', array( 'url(', $out, ')' ) );
